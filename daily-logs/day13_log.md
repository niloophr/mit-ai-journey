# Day 13 – November 29, 2025 (8 Azar 1404)
![MIT AI Journey - Day 13](https://img.shields.io/badge/MIT_AI_Journey-Day_13_Complete-0066CC?style=for-the-badge&logo=github)
## Completed ✅
- [x] **English Reading** – The Midnight Library: read several pages + highlighted new vocab
- [x] **Writing & Vocab** – noted unknown words from the book
- [x] **Python** – wrote 2 complete scripts using if / elif / else logic
- [x] **No zero day** – kept the chain alive

## Time Spent
⏱️ ~60 minutes of solid work  
📍 Tehran, Iran

> Day 13 – literature, vocabulary, and code. Every day counts.  
> Chain is now **13 days strong and unbreakable**.  
> From Tehran to MIT – no zero days, no excuses.

Streak: 13 days
